...
repod.thread_updater = {
	...
	update: function() {
		if (repod.thread_updater.config.enabled && $(".post.op a[title='Quote']").length == 1) {
			$("a:contains('Bottom')").after("] [<a class='update_button' href=''>Update</a></span>] [<label for='updater_checkbox'><input type='checkbox' id='updater_checkbox' "+((repod.thread_updater.config.auto_update) ? "checked" : "")+">Auto</label>&nbsp;<span class='updater_timer'></span><span class='updater_status'>");
		}
	...
	load_thread_url: function(url) {
		...
		$.ajax({url:url,success:function(result){
			var counter = 0;
                        $("#board").replaceWith($(result).find("#board").clone());
			if (counter > 0) {
...
$(window).scroll(function() {
	if(Math.round($(window).scrollTop() + $(window).height()) == repod_jsuite_getDocHeight()) {
