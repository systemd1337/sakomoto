...
function set_inputs(id) {
      if (document.getElementById(id)) {
           with(document.getElementById(id)) {
                if(!nam.value) nam.value = getCookie("name");
                if(!ema.value) ema.value = getCookie("email");
...
function reppop(a) {
    day = new Date();
    id = day.getTime();
    window.open(a, id, "toolbar=0,scrollbars=0,location=0,status=1,menubar=0,resizable=1,width=380,height=450");
    return false;
}
...
function popup(vars) {
    ...
    id = day.getTime();
    var newWindow;
    var props = 'scrollBars=yes,resizable=no,toolbar=no,menubar=no,location=no,directories=no,width=430,height=550';
