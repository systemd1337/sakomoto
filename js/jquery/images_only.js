...
repod.hide_images = {
	init: function() {
		this.config = {
			...
			reverse: repod.suite_settings && !!repod_jsuite_getCookie("r_jq_hide_nonimages") ? repod_jsuite_getCookie("r_jq_hide_nonimages") === "true" : false
		}
		...
		repod.suite_settings && repod.suite_settings.info.push({menu:{category:'Images',read:this.config.reverse,variable:'r_jq_hide_nonimages',label:'Hide image posts',hover:'Exluding OP, hide posts that contain images'}});
	...
	update: function() {
		...
		repod.hide_images.config.reverse && $("td.reply:has(a > img.postimg)").closest("table").css("display","none");
	}
};